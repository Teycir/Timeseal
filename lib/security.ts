...
import { isOriginAllowed, isRefererAllowed } from "./appConfig";
...
export function setSecurityEnv(env: SecurityEnv): void {
  cachedEnv = env;
}
...
import {
  ...
  MAX_CONCURRENT_TRACKER_ENTRIES,
  NONCE_EXPIRY,
  PULSE_TOKEN_WINDOW,
} from "./constants";
...
export function detectSuspiciousPattern(ip = "", sealId = "") {
...
export async function generatePulseToken(
  sealId: string,
  secret: string,
): Promise<string> {
  const timestamp = Date.now();
  const data = `${sealId}:${timestamp}`;
  const signature = await hmacSign(data, secret);
  return `${data}:${signature}`;
}

export async function validatePulseToken(
  token: string,
  sealId: string,
  secret: string,
): Promise<boolean> {
  ...
  const [tokenSealId, timestamp, signature] = parts;
  ...
  const data = `${tokenSealId}:${timestamp}`;
  return await hmacVerify(data, signature, secret);
}

export async function validatePulseOperation(
  token: string,
  operationNonce: string,
  db: import("./database").DatabaseProvider,
): Promise<boolean> {
  return await checkAndStoreNonce(operationNonce, db);
}
...
export function sanitizeError(error = new Error()) {
